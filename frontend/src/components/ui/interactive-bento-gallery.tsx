    ...
    return (
        <div className="relative w-full h-full group" onClick={onClick}>
            <img
                ...
                className={`${className} object-cover w-full h-full ${item.objectPosition || 'object-center'}`} // Style the image
                loading="lazy" // Lazy load the image for performance
                decoding="async" // Decode the image asynchronously
            />
            {/* Elegant slide-up overlay that prevents clutter on narrow cards */}
            <div className="absolute inset-0 bg-gradient-to-t from-gray-900/90 via-gray-900/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl" />
            <div className="absolute inset-x-0 bottom-0 p-3 sm:p-5 translate-y-6 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end pointer-events-none">
                <h3 className="text-white font-bold text-sm sm:text-base leading-tight mb-1">{item.title}</h3>
                <p className="text-gray-300 text-xs sm:text-sm line-clamp-2 mb-2 sm:mb-3">{item.desc}</p>
                <div className="flex items-center text-white font-medium text-xs sm:text-sm">
                    Read more <ArrowRight className="ml-1 w-3 h-3 sm:w-4 sm:h-4 transition-transform group-hover:translate-x-1" />
                </div>
            </div>
        </div>
    );
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({ mediaItems, title, description }) => {
    ...
    return (
        <div className="container mx-auto px-4 pb-8 w-full max-w-7xl overflow-hidden">
            <div className="mb-8 text-center">
                <motion.h1
                    className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-gray-900 whitespace-nowrap"
            ...
            <AnimatePresence mode="wait">
                {selectedItem ? (
                    ...
                    <motion.div
                        className="flex flex-wrap md:flex-nowrap justify-center gap-4 md:gap-6 overflow-x-auto pb-8"
                        initial="hidden"
                        animate="visible"
                        exit="hidden"
                        variants={{
                    ...
                    >
                        {items.map((item, index) => (
                            <motion.div
                                ...
                                className="relative cursor-move transition-all duration-300 hover:shadow-xl hover:-translate-y-1 w-[230px] h-[480px] flex-shrink-0 overflow-hidden rounded-2xl bg-gray-100 shadow-md group"
